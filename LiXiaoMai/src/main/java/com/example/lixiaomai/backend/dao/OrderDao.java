...
public class OrderDao {
    ...
    public boolean addOrder(Order order){
        try{
            Connection conn = DatabaseUtils.getConnection();
            String sql = "INSERT INTO `ORDER` (CID, SID, GID, GOODSNUM, ENDTIME, STARTTIME, TOTAL, STATUS, SNAME, DISCOUNTNUM, DID, CNAME, DELIVERID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            return runner.update(conn, sql,
                    order.getCId(), order.getSId(), gId, goodsNum, order.getEndTime(), order.getStartTime(), order.getTotal(), order.getStatus(), order.getSName(), discountNum, discountId, order.getCName(), order.getDeliverId()) > 0;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
